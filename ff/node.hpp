...
protected:
    ...
    virtual int create_input_buffer(int nentries, bool fixedsize=true) {
        if (in) return -1;
        std::cout<< "THIS IS MY QUEUE INPUTSIZE" << nentries<< std::endl;
    ...
    virtual int create_output_buffer(int nentries, bool fixedsize=false) {
        if (out) return -1;
        std::cout<< "THIS IS MY QUEUE outPUTSIZE" << nentries<< std::endl;
...
public:
    ...
    /*Mehdi:FF-VIP*/
    pthread_mutex_t readWriteProfile;
    /*Mehdi:FF-VIP*/
    ...
    #if defined(FASTFLOW_OPENCL)
    void prepare_openCL(ff_oclParam* oclparam)
    {
        clEnvironment::instance()->prepare(oclparam);
    }
    #endif
  ...
  // Mehdi
    ...
    virtual void set_actuators(JSONNode actuators){
        if(actuators.type()!=JSON_NULL)
        {
            ...
            {
                 int val = jniter->as_int();
                 set_status(val);
           //     JSONNode::const_iterator it =actuators.find_nocase("Assigned_Processor");
            //    if(it!=actuators.end() && it->as_int()!=-1) (setAffinity((it->as_int())));
            }
        }
    }
    // afte switch the node is going to be considered as new now and all data from now on is going to be calculated
   
    void set_status(int val)
    {
        if (val==0) cmpStatus=false;
        else cmpStatus=true;
        //std::cout << "THIS IS THE STATUSS!!!!!!!!!!!!!!!!!!!!!!!"<< cmpStatus<<std::endl;
    }
    /*Mehdi:FF_VIP*/
    ...
    std::string get_node_address(){
        if (!(node_address.empty()))  
            ...
            return get_node_address(get_parent_name());
    }
    void set_node_address(std::string address){
        node_address=address;   
    }

    std::string get_node_address(std::string parent_name){
        int status;
        std::string compName = abi::__cxa_demangle(typeid(*this).name(), 0,0,&status );
        std::transform(compName.begin(), compName.end(), compName.begin(), ::tolower);
        int local_id = ff_node::get_my_id(); 
        if (local_id<0) local_id=0;
        std::string my_name = parent_name + "/" + compName + "/" +  static_cast<ostringstream*>( &(ostringstream() << local_id) )->str();
        std::transform(my_name.begin(), my_name.end(), my_name.begin(), ::tolower); 
        set_node_address(my_name);
        return my_name;
    }
    ...
    void set_parent_name(std::string address){
        parent_name=address;   
    }
    /*Mehdi:FF_VIP*/  
    virtual void setDataSize(unsigned long long dataSize){
        this->dataSize=dataSize;
    }
    virtual unsigned long long getDataSize(){
    ...
    virtual int  get_ondemand(int i){}//mehdi
    virtual void set_ondemand(std::vector<int> v){} //mehdi
    virtual int run_and_wait_end() {
        std::cout<<"Wrong one HERE"<<std::endl;
    }//mehdi
    ...
    JSONNode getSamples(unsigned long long dataSize,const std::vector<ticks>& collectedSamples, const char* arr_name){ 
        JSONNode jnArray(JSON_ARRAY);
        jnArray.set_name(arr_name);
        for(std::vector<ticks>::const_iterator iter=collectedSamples.begin(); iter< collectedSamples.end(); ++iter){
            jnArray.push_back(JSONNode("",*iter));
        }
        return jnArray;
    };

    JSONNode getSamples(unsigned long long dataSize,const std::vector<ticks>& collectedSamples, const JSONNode& prevObject, const char* arr_name){ 
    JSONNode jnArray(JSON_ARRAY);
    jnArray.set_name(arr_name);
    for(std::vector<ticks>::const_iterator iter=collectedSamples.begin(); iter< collectedSamples.end(); ++iter){
        jnArray.push_back(JSONNode("",*iter));
    ...
    virtual int get_modular_mode()
    {
        int m_m= modular_mode; 
        return m_m;
    }
    /*Mehdi:FF-VIP*/
    virtual void set_modular_mode(int val)
    {
        this->modular_mode=val;   
    }

     void set_swith_status()
    {
        //if (val==0) cmpStatus=false;
        //else cmpStatus=true;
        active_time=0;
        switched_data=taskcnt;
        //std::cout << "THIS IS THE STATUSS!!!!!!!!!!!!!!!!!!!!!!!"<< cmpStatus<<std::endl;
    }
    virtual JSONNode get_sensors(){
        ...
        pthread_mutex_lock(&readWriteProfile);
        collectedSamples.swap(svcTicksInterval);
        pthread_mutex_unlock(&readWriteProfile);
        sensors.push_back(JSONNode("Component_Name", abi::__cxa_demangle(typeid(*this).name(), 0,0,&status )));
        sensors.push_back(JSONNode("Component_Type", "sequential"));
        //sensors.push_back(JSONNode("Local_ID", get_my_id()));
        sensors.push_back(JSONNode("Node_Address", get_node_address()));
        sensors.push_back(JSONNode("Elapsed_Time", getElapsedTime()));
        sensors.push_back(JSONNode("Assigned_Processor", getCPUId()));
      //  sensors.push_back(JSONNode("Data_Size", dataSize));
        sensors.push_back(JSONNode("Processed_Tasks", (taskcnt-switched_data)));
        
        bool inpresent  = (get_in_buffer() != NULL);
        inpresent? sensors.push_back(JSONNode("Queue_Input", in->length())): 
            sensors.push_back(JSONNode("Queue_Input", -1)); 
        bool outpresent = (get_out_buffer() != NULL);
        outpresent? sensors.push_back(JSONNode("Queue_Output", out->length())):
       ...
       // sensors.push_back(JSONNode("Last_SVC_Time", 0));
        ...
        get_modular_mode()!=0 ? sensors.push_back(JSONNode("Sampling_Rate", (1.0/get_modular_mode()))) 
        : sensors.push_back(JSONNode("Sampling_Rate", 0));
        sensors.push_back(getSamples(dataSize,collectedSamples, "Component_Time_Distribution"));
        ...
        (!(end_recieved))? sensors.push_back(JSONNode("End_Received", 0)):
        sensors.push_back(JSONNode("End_Received", 1));// cheange to 1 for fase changing
        sensors.set_name("Sequential");
        return sensors;
    }
    /*virtual mObject sensor(){
        ...
        collectedSamples.swap(svcTicksInterval);
        pthread_mutex_unlock(&readWriteProfile);
        cObj["FF::Node_Subclass"]=abi::__cxa_demangle(typeid(*this).name(), 0,0,&status );
        cObj["Type"]="FF::NODE";
        cObj["Local_ID"]=get_my_id();
        cObj["Assigned_Processor"]=getCPUId();
        cObj["Data_Size"]= dataSize;
        cObj["Total_Number_Of_Tasks"]= taskcnt;
        cObj["Total_SVC_Ticks"]= static_cast<uint64_t>(tickstot);
        cObj["Minimum_SVC_Tick"]= static_cast<uint64_t>(ticksmin);
        cObj["Maximum_SVC_Tick"]= static_cast<uint64_t>(ticksmax);
        if(get_modular_mode()!=0)
            cObj["Sampling_Rate"]= 1.0/get_modular_mode();
        else
            cObj["Sampling_Rate"]=0;
        cObj["Samlped_SVC_Tick_Distribution"]= getSamples(dataSize,collectedSamples);
        cObj["Push_Delay_Count"]=pushwait;
        cObj["Push_Delay_Ticks"]= static_cast<uint64_t>(lostpushticks);
        cObj["Pop_Delay_Count"]= popwait;
        cObj["Pop_Delay_Ticks"]= static_cast<uint64_t>(lostpopticks);
        return cObj;
  }*/
    /*Mehdi:FF-VIP*/
   //this function is only used by threadworker in which it already has the read lock write. then there is no need to furthure lock here
...
protected:
    ...
    ff_node():in(0),out(0),myid(-1),CPUId(-1),
              ...
              skip1pop(false), in_active(true), thread(NULL),callback(NULL),
              barrier(NULL),end_callback(NULL),end_callback_param(NULL),EXIT(false) {
...
#endif
        /*Mehdi:FF-VIP*/
        pthread_mutex_init(&readWriteProfile, NULL);
        /*Mehdi:FF-VIP*/
    ...
    public:
        ...
        inline bool push(void * task) {
            ...
            while (! filter->push(task)) { 
                ...
                //} else 
                // uncoment the following line for correct change yourself
                #if defined(TRACE_FASTFLOW)
                filter->last_time=0.01;// in order to be allocated some task by dlt you CANNOT SET  IT TO ZEORALSO IT IS NOT ACCURATE BUT IT WILL WORK, because in the next run it will correct itself
                #endif
        ...
        void* svc(void * ) {
            ...
            do {
                ...
                double time0 =filter->getElapsedTime();//mehdi
#endif
                ret = filter->svc(task);
#if defined(TRACE_FASTFLOW)
                filter->active_time += (filter->getElapsedTime()-time0);//mehdi
                filter->last_time=(filter->getElapsedTime()-time0);//mehdi;

                register ticks diff=(getticks()-t0);
                
                /*Mehdi:FF-VIP*/
                if (ret && (ret != (void*)FF_EOS) && (ret != (void*)FF_EOS_NOFREEZE)) {
                    if((filter->get_modular_mode()!=0)&&((filter->taskcnt)!=0)&&(((filter->taskcnt)%(filter->get_modular_mode()))==0)){
...
private:
    ...
    ticks         tickstot;
    /*Mehdi:FF_VIP*/
    std::vector<ticks> svcTicksInterval;
    double active_time;
    /*Mehdi:FF_VIP*/
