    ...
    { 
        if(first_flag)
        {
           /* pthread_mutex_lock(&(mutex_set_policy));
            ...
            myOclId=clEnvironment::instance()->getOCLID();
            ...
            {
                std::cout<<"**************PEI is not used. A static greedy algorithm is used to allocate openCL component******************: "<<std::endl;
		        int GPUdevId =clEnvironment::instance()->getGPUDevice();
		        std::cout<<"**numGPU*****: "<< clEnvironment::instance()->numGPU<< " MYOCLID: "<<myOclId << std::endl;
                if( (GPUdevId !=-1) && ( myOclId<clEnvironment::instance()->numGPU))
                { 
                	std::cout << "I have allocated the GPU device this is the id"<< GPUdevId<< std::endl;
                	baseclass_ocl_node_deviceId=clEnvironment::instance()->clDevices.at(GPUdevId);
                }

                else
                {	// fall back to CPU either GPU has reached its max or there is no GPU available
                	std::cout << "I have allocated the CPU device as either no GPU device is available or no gpu slot is available "<< GPUdevId<< std::endl;
                	int CPUdevId =clEnvironment::instance()->getCPUDevice();
                	baseclass_ocl_node_deviceId=clEnvironment::instance()->clDevices.at(CPUdevId);
                }
