...
#include <ff/node.hpp>
/*Mehdi:FF-VIP*/
#include <ff/PEI/adaptiveLoadbalancer.hpp>
...
protected:
    ...
    inline int prepare() {
        for(int i=0;i<nworkers;++i)
        {
            if(in_buffer_entries != DEF_IN_BUFF_ENTRIES)
            {
                if (workers[i]->create_input_buffer((ondemand ? ondemand: get_worker_queue_bound(i)), 
                ...
                if (workers[i]->create_output_buffer((ondemand ? ondemand:get_worker_queue_bound(i)+nworkers), 
            ...
            }else
            {
                ...
                if (collector || lb->masterworker() || collector_removed) 
                    // NOTE: force unbounded queue if masterworker
                    if (workers[i]->create_output_buffer((ondemand ? ondemand:(out_buffer_entries/nworkers + DEF_IN_OUT_DIFF)), 
                                                     (lb->masterworker()?false:fixedsize))<0)
                        return -1;
            }
...
public:
    ...
    int get_worker_queue_bound(int i)//mehdi
    {
        return lb->get_worker_queue_bound(in_buffer_entries, i);
    }
    void set_worker_queue_bound(std::vector<int> v)//mehdi
    {
        lb->set_worker_queue_bound(v);
    }
    /**
 ...
 // Mehdi
    virtual double getElapsedTime(){
        ...
        return diffmsec(tcurrent,lb->getstarttime());
    }
    //Mehdi
    ...
    void set_actuators(JSONNode actuators)
    {
        ...
        {   
            ...
            {
                lb->set_actuators(actuators);
                for(int i=0; i<nworkers; i++)
                {
                    workers[i]->set_actuators(actuators);
                }
                gt->set_actuators(actuators);

            }
            else
            {
                lb->set_actuators((actuators.find_nocase("1ToNCom"))->as_node());

                JSONNode::const_iterator it =actuators.find_nocase("Parallel");
                if(it==actuators.end())
                {
                    it =actuators.find_nocase("MISD");
                }
               // JSONNode::const_iterator it;
               // parallel? 
                //it =actuators.find_nocase("Parallel"): it =actuators.find_nocase("MISD");

                JSONNode workers_array = it->as_array();
                JSONNode workersObject = workers_array[0].as_node();

                //JSONNode::const_iterator it =actuators.find_nocase("Parallel");
                int j=0;
                for(int i=0; i<workersObject.size(); i++)
                {
                    if(workersObject[i].type()==JSON_NODE)
                    {
                        workers[j]->set_actuators(workersObject[i].as_node());
                        j++;
                    }
                }
                gt->set_actuators((actuators.find_nocase("NTo1Com"))->as_node());
            }
        }
    }

    /*Mehdi:FF-VIP   */
    /*void actuator(mObject instObj){
        mObject lbObj;
        mObject gtObj;
        ...
        workers[i]->actuator(wobj);
         ...
         mv =find_value(instObj, "Collector");
         if(!(mv.is_null())) {gtObj= mv.get_obj();}
         gt->actuator(gtObj);
    }*/
    /*Mehdi:FF-VIP   */
    void setDataSize(unsigned long long dataSize){
        lb->setDataSize(dataSize);
        for(int i=0;i<nworkers;++i){
            workers[i]->setDataSize(dataSize);
        }
        if(collector){
            gt->setDataSize(dataSize);
        }
    }
    /*Mehdi:FF-VIP   */
    bool getEXIT(){
        if(!(lb->getEXIT())) return false;  
        for(int i=0; i<nworkers; i++){
            if (!(workers[i]->getEXIT())) {/*std::cout<<"workers: "<< i <<std::endl; */return false;}
        }
        if(collector){
            if (!(gt->getEXIT())){/* std::cout<<"gt"<<std::endl; */ return false;}
        }
        return true;
    }
    ...
    void set_modular_mode(int val){
        lb->set_modular_mode(val);
        for(int i=0; i<nworkers;i++){
            workers[i]->set_modular_mode(val);
        }
        gt->set_modular_mode(val);  
    }

    /*Mehdi:FF-VIP*/
    void set_type(bool tp)
    {
        parallel=tp;
    }
    bool get_type()
    ...
    JSONNode get_sensors(){
        JSONNode sensors(JSON_NODE);
        int status;         
        sensors.push_back(JSONNode("Component_Name", abi::__cxa_demangle(typeid(*this).name(), 0,0,&status )));
        sensors.push_back(JSONNode("Component_Type", "farm"));
        sensors.push_back(JSONNode("Node_Address", get_node_address()));
       // sensors.push_back(JSONNode("Local_ID", ff_node::get_my_id()));
        lb->set_parent_name(get_node_address());
        sensors.push_back(lb->get_sensors());
        JSONNode farmWorkers(JSON_ARRAY);
        JSONNode frmWorkers (JSON_NODE);
        parallel?
            farmWorkers.set_name("parallel"): farmWorkers.set_name("MISD") ;
        for(int i=0;i<nworkers;++i) {
            workers[i]->set_parent_name(get_node_address());
            frmWorkers.push_back(workers[i]->get_sensors());
        }
        farmWorkers.push_back(frmWorkers);
        sensors.push_back(farmWorkers);
        gt->set_parent_name(get_node_address());
        sensors.push_back(gt->get_sensors());
        sensors.push_back(JSONNode("Elapsed_Time", getElapsedTime()));
        sensors.set_name("Composition");
        return sensors;
    }
    /*Mehdi:FF-VIP   */
    /*virtual  mObject sensor(){
        ...
        int status;
        fObj["FF::Node_Subclass"]=abi::__cxa_demangle(typeid(*this).name(), 0,0,&status );
        fObj["Type"]="FF::FARM";
...
protected:
    ...
    int create_input_buffer(int nentries, bool fixedsize) {
        ...
        //std::cout<< "THIS IS THE VALUE SET FOR EMITTER:"<<nentries<< std::endl;
        lb->set_in_buffer(in);
