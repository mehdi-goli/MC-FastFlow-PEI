...
int main(int argc, char * argv[])
{
   ...
   // if (argc<3) 
    //{
     //   std::cerr << "insert .json path containing skeleton specification and .so Libbrary path containing executable functions for: " << argv[0] << "\n";
      //  return -1;
    //}
    
    //const char* json_address=argv[1];
    //const char* lib_address=argv[2];
    std::string kernel_path("/users/mehdi/Thesis/fastflow-2.0.2-paraphrase/examples/Image_processing/Kernels/SobelFilter_Kernels.cl");
    ...
    //ff_farm<adaptive_loadbalancer, ff_gatherer> * s_farm = new ff_farm<adaptive_loadbalancer, ff_gatherer>();
    ...
    //PROFILE(root, sm, pr); where sm determine a vlue for sampling mode and pr determine a value for priority
